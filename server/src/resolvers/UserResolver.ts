                        ...
                        function (error, data) {
                            if (error) {
                                ...
                                transporter.sendMail({
                                    from: `Support Team <support@${process.env.DOMAIN}>`,
                                    to: email,
                ...
                ejs.renderFile(
                    ...
                    function (error, data) {
                        if (error) {
                            ...
                            transporter.sendMail({
                                from: `Support Team <noreply@${process.env.DOMAIN}>`,
                                to: user?.email,
    ...
    async deleteAccount(
        ...
        if (!payload) {
            ...
            if (payload.role === "admin" && (adminUsers.length - 1) === 0) {
                status = "You can't delete your account because you are the only admin of the blog.";
                ok = false;
            } else {
                await User.remove(user!).then(() => {
